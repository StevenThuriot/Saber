...
namespace Moon.Testing
{
	...
	/// </summary>
	[DebuggerDisplay("Moon's Testing helper.")]
	public static class ExAssert
	{
		...
		/// <exception cref="MoonAssertException">The assertion failed.</exception>
		...
		private static void AreEqual(string expected, string actual)
		{
			if (expected != actual)
			{
				throw new MoonAssertException(
						Settings.Moon.Language.WrongException,
						expected,
						actual);
			}
		}
		...
		private static void AreEqual(Type expected, Type actual)
		{
			...
			{
				throw new MoonAssertException(
						Settings.Moon.Language.WrongMessage,
						expected.Name,
						actual.Name);
			}
		}
		...
		private static void Fail(string message)
		{
			throw new MoonAssertException
					(
						Settings.Moon.Language.ExceptionNotThrown,
						message
					);
		}
	}
